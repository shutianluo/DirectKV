/*
 * SMPv2 CPU-KV CUDA Extension (SM90, bf16, GH200 NVLink-C2C)
 * ===========================================================
 ...
 *
 * Python signature:
 *   smpv2_fwd(X, Wk, Wv, Q, K_cpu, V_cpu, cos_sin,
...
#include "parallelism_sm_v2_cpu_kv_kernel.cuh"
...
// --------------------------------------------------------------------------
// TMA descriptor helpers (BF16, matching test_smpv2_cpu_kv.cu)
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
static torch::Tensor smpv2_fwd_impl(
    torch::Tensor X,
    torch::Tensor Wk,
    torch::Tensor Wv,
    torch::Tensor Q,
...
struct LayerTmaSet {
    ...
    // GQA bookkeeping (set in smpv2_init_tma, read in smpv2_fwd_graph)
...
// --------------------------------------------------------------------------
// smpv2_init_tma: pre-build all 6 TMA descriptors for one layer using
    ...
    torch::Tensor V_max)
{
    ...
    alloc_layer_tma(s);
    // Store GQA dims so smpv2_fwd_graph can build params without Wk/Wv
    s.nh_kv     = NH;
    s.nh_q      = NH_q;
    s.gqa_ratio = NH_q / NH;
    ...
    auto cu_ok = [](CUresult r, const char* name) {
        TORCH_CHECK(r == CUDA_SUCCESS,
            "smpv2_init_tma: cuTensorMapEncodeTiled failed for ", name, ": err=", (int)r);
    };
    ...
    TORCH_CHECK(ea == cudaSuccess, "smpv2_init_tma: pre-alloc O_run failed: ", cudaGetErrorString(ea));
    TORCH_CHECK(eb == cudaSuccess, "smpv2_init_tma: pre-alloc LSE_run failed: ", cudaGetErrorString(eb));
}

// --------------------------------------------------------------------------
// smpv2_fwd_graph: graph-safe forward (no TMA rebuild, reads num_n_blocks
// from device memory). Requires smpv2_init_tma to have been called first.
//
...
// All shapes must match those used in smpv2_init_tma for this layer.
// --------------------------------------------------------------------------
torch::Tensor smpv2_fwd_graph(
    int64_t layer_id,
    torch::Tensor X_pad,
    torch::Tensor Q_pad,
    torch::Tensor O_out,
    ...
    c10::optional<torch::Tensor> req_pool_indices)
{
    ...
    TORCH_CHECK(g_cg_tma.count(lid) > 0,
        "smpv2_fwd_graph: layer ", lid, " not initialized — call smpv2_init_tma first");
    ...
    // Read NH_kv and NH_q from the TmaSet stored during smpv2_init_tma.
    ...
    cudaError_t err =
        sm_parallel_v2_cpu_kv::launch_sm_parallel_v2_cpu_kv<Traits>(p, stream);
    TORCH_CHECK(err == cudaSuccess,
        "smpv2_fwd_graph: launch failed: ", cudaGetErrorString(err));
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.doc() = "SMPv2 CPU-KV: fused X->KV projection + Neox RoPE + attention (SM90 bf16)";
    m.def("smpv2_fwd", &smpv2_fwd,
        py::arg("X"),
        py::arg("Wk"),
        py::arg("Wv"),
        py::arg("Q"),
    ...
    m.def("smpv2_init_tma", &smpv2_init_tma,
        py::arg("layer_id"),
        py::arg("X_max"),
        py::arg("Wk"),
        py::arg("Wv"),
        ...
        "Must be called once per layer before smpv2_fwd_graph.\n"
        "All tensor shapes must be the max shapes that will be used during graph replay.");

    m.def("smpv2_fwd_graph", &smpv2_fwd_graph,
        py::arg("layer_id"),
        py::arg("X_pad"),
        py::arg("Q_pad"),
        py::arg("O_out"),
        ...
        "Graph-safe SMPv2 forward (no TMA rebuild).\n"
        ...
        "smpv2_init_tma must have been called first for this layer_id.\n"
