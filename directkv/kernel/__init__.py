...
Two extensions are provided:
  qcentric_fwd  — qcentric_attn_kernel (original, Q-centric attention)
  directkv_fwd  — sm_parallel_v2_cpu_kv_fwd_kernel (fused projection + RoPE + attention)
...
_EXT_SOURCE = os.path.join(_THIS_DIR, "qcentric_extension.cu")

# DirectKV extension — kernel sources are in csrc/ relative to the repo root
_REPO_ROOT = os.path.dirname(os.path.dirname(_THIS_DIR))   # DirectKV_AE-main/
_DEFAULT_DIRECTKV_KERNEL_SRC = os.path.join(_REPO_ROOT, "csrc")
_DIRECTKV_EXT_SOURCE = os.path.join(_THIS_DIR, "directkv_extension.cu")
...
# ---------------------------------------------------------------------------
# DirectKV CPU-KV extension  (fused projection + Neox RoPE + attention, SM90)
# ---------------------------------------------------------------------------

@lru_cache(maxsize=1)
def _load_directkv_extension():
    """JIT-compile the directkv_extension.cu (SM90, bf16, once per process)."""
    cutlass_inc = _find_cutlass_include()
    if cutlass_inc is None:
        raise RuntimeError(
            "CUTLASS headers not found. Set CUTLASS_INCLUDE_DIR to your CUTLASS "
    ...
    kernel_src = os.environ.get("DIRECTKV_KERNEL_SRC", _DEFAULT_DIRECTKV_KERNEL_SRC)
    if not os.path.isdir(kernel_src):
        raise RuntimeError(
            f"DirectKV kernel source directory not found: {kernel_src!r}. "
            f"Set DIRECTKV_KERNEL_SRC to the DirectKV csrc/ directory."
        )

    if not os.path.isfile(_DIRECTKV_EXT_SOURCE):
        raise RuntimeError(
            f"directkv_extension.cu not found at {_DIRECTKV_EXT_SOURCE!r}."
        )
...
def directkv_fwd(
    ...
    """
    DirectKV fused forward pass: X->K,V projection + Neox RoPE + attention.
    ...
    """
    ext = _load_directkv_extension()
    return ext.directkv_fwd(
...
def directkv_init_tma(
    ...
    Subsequent directkv_fwd_graph calls for this layer_id use these descriptors.
    """
    ext = _load_directkv_extension()
    ext.directkv_init_tma(layer_id, X_max, Wk, Wv, Q_max, K_max, V_max)


def directkv_fwd_graph(
    layer_id: int,
    ...
    """
    Graph-safe DirectKV forward (skips TMA descriptor creation).
    ...
    Requires directkv_init_tma(layer_id, ...) to have been called first.
    Returns O_out (same tensor) for convenient chaining.
    """
    ext = _load_directkv_extension()
    return ext.directkv_fwd_graph(
