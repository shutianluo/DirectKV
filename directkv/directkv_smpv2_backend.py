"""
DirectKV SMPv2 Attention Backend
=================================
...
# Backend
# ---------------------------------------------------------------------------
class DirectKVSmpV2Backend(AttentionBackend):
    ...
    def __init__(self, model_runner: "ModelRunner"):
        ...
        try:
            from sglang_ext.directkv_kernel import (
                smpv2_fwd as _smpv2_fwd,
                smpv2_init_tma as _smpv2_init_tma,
                smpv2_fwd_graph as _smpv2_fwd_graph,
            )
        except ImportError:
        ...
        self._smpv2_fwd       = _smpv2_fwd
        self._smpv2_init_tma  = _smpv2_init_tma
        self._smpv2_fwd_graph = _smpv2_fwd_graph
        ...
        if not self._use_kernel:
            logger.warning(
                f"[DirectKV-SMPv2] NH_q={self.num_q_heads} is not a multiple of "
    ...
    def _check_args(self, runner: "ModelRunner") -> None:
        ...
        if self.head_dim != 128:
            raise ValueError(
                f"directkv-smpv2 requires head_dim=128, got {self.head_dim}. "
                "Use a compatible model (e.g. OPT-6.7B, Llama-3-8B with GQA mod)."
            )
        if runner.model_config.dtype not in (torch.bfloat16,):
            raise ValueError("directkv-smpv2 requires bfloat16 model weights.")
    ...
    def _ensure_pool(self) -> None:
        ...
        logger.info(f"[DirectKV-SMPv2] Pool: {self._pool}")
    ...
    def _setup_hooks(self) -> None:
        ...
        logger.info(
            f"[DirectKV-SMPv2] Hooked {registered} layers; "
    ...
    def _extract_weights(
        ...
        logger.warning(f"[DirectKV-SMPv2] Layer {layer_id}: cannot extract weights.")
        return None, None
    ...
    def init_cuda_graph_state(self, max_bs: int, max_num_tokens: int) -> None:
        ...
        logger.info(
            f"[DirectKV-SMPv2] CUDA graph state: max_bs={max_bs}, "
    ...
    def forward_decode(
        ...
        # The SMPv2 kernel requires all past 64-blocks to be fully populated.
        ...
        # Kernel path: bucket 64-aligned requests → one smpv2_fwd call per bucket
        if kernel_idxs:
